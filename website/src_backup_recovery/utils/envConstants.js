...
export function getWorkflowStages(caseKey = 'CASE_A') {
  const targetCase = DEMO_CASES[caseKey] || DEMO_CASES.CASE_A;
  const isCaseA = caseKey === 'CASE_A';
  const center = [targetCase.spillCenter.lat, targetCase.spillCenter.lon];
  const originCenter = [targetCase.originCenter.lat, targetCase.originCenter.lon];
  const highlightVessel = targetCase.topVessel;
